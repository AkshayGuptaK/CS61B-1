...
class Model {
    ...
    void copy(Model model) {
        if (model == this) {
            ...
            this.numCols = model.cols();
            this.numRows = model.rows();
            this.centers = new ArrayList<>(model.centers);
            this.marksList = new HashMap<>();
            for (Map.Entry<Place, Integer> entry : model.marksList.entrySet()) {
                this.marksList.put(entry.getKey(), entry.getValue());
            }
            this.boundaries = model.boundaries;
            this.perimeter = model.perimeter;
            this.board = new int[xlim() - 1][ylim() - 1];
    ...
    private int numRows;
    /**
     * Creating 2D array for game board.
     */
    private int[][] board;
    /**
     * Dictionary of Place objects and their marks.
     */
    private HashMap<Place, Integer> marksList = new HashMap<>();
    /**
     * Hash set of Place objects that are boundaries on the board.
     */
    private HashSet<Place> boundaries = new HashSet<>();
    /**
     * Hash set of Place object that are on the perimeter of the board.
     */
    private HashSet<Place> perimeter = new HashSet<>();
    /**
     * List of Place objects that are centers.
     */
    private List<Place> centers = new ArrayList<>();
    ...
    void init(int cols, int rows) {
        this.numCols = cols;
        this.numRows = rows;
        this.board = new int[rows * 2 + 1][cols * 2 + 1];
        ...
        for (int row = 0; row < ylim(); row++) {
            for (int col = 0; col < xlim(); col++) {
                if (col == 0
                        ...
                        || col == xlim() - 1
                        || row == ylim() - 1) {
                    if (!isIntersection(col, row)) {
                        this.perimeter.add(pl(col, row));
    ...
    boolean isBoundary(int x, int y) {
        return boundaries.contains(pl(x, y)) || perimeter.contains(pl(x, y));
    }
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        HashSet<Place> copy = new HashSet<>(region);
        for (Place cell : copy) {
            assert isCell(cell);
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (boundary != center && isCenter(boundary)) {
                    return false;
                }
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (center != intersection && isCenter(intersection)) {
                    return false;
                }
            }
        }
    ...
    HashSet<Place> findGalaxy(Place center) {
        ...
        if (this.perimeter.contains(pl(center.x, center.y))) {
            return null;
        } else if (isHoriz(center.x, center.y)) {
            accreteRegion(pl(center.x, center.y - 1), galaxy);

        } else if (isCell(center.x, center.y)) {
            accreteRegion(center, galaxy);

        } else if (isIntersection(center.x, center.y)) {
            accreteRegion(pl(center.x + 1, center.y + 1), galaxy);

        } else if (isVert(center.x, center.y)) {
            accreteRegion(pl(center.x - 1, center.y), galaxy);
        }
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        ArrayList<Place> regionL = new ArrayList<>(region);

        int loop = 0;
        while (loop < region.size()) {
            loop = region.size();
            List<Place> cen = unmarkedSymAdjacent(center, regionL);
            for (Place c : cen) {
                region.add(c);
                regionL.add(c);
            }
    ...
    void toggleBoundary(int x, int y) {
        if (isEdge(x, y) && !boundaries.contains(pl(x, y))) {
    ...
    int mark(int x, int y) {
        if (!isCell(x, y)) {
            return -1;
        } else if (!marksList.containsKey(pl(x, y))) {
            return 0;
        } else {
            return marksList.get(pl(x, y));
        }
    }
    ...
    void markAll(int v) {
        assert v >= 0;
        for (int rows = 0; rows < ylim(); rows++) {
            for (int columns = 0; columns < xlim(); columns++) {
                marksList.put(pl(rows, columns), v);
            }
        }
    }
    ...
    Place opposing(Place p0, Place p) {
        if (!isCell(p0.x - (p.x - p0.x), p0.y - (p.y - p0.y))) {
            return null;
        } else {
            return pl(p0.x - (p.x - p0.x), p0.y - (p.y - p0.y));
        }
    }
