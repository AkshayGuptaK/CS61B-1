...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("manual\\s+(([wW][hH][iI][tT][eE])|"
                +
                "([bB][lL][aA][cC][kK]))$", this::doManual),
        new Command("auto\\s+(([wW][hH][iI][tT][eE])|"
                +
                "([bB][lL][aA][cC][kK]))$", this::doAuto),
        new Command("[a-j]([1-9]|10)(-| )*[a-j]{1}([1-9]|10)"
                +
                "(\\(| )*[a-j]{1}([1-9]|10)\\)*", this::doMove)
    };
    ...
    /** Manual player. MAT is the matched string. */
    ...
    /** AI player. MAT is the matched string. */
    ...
    private void doMove(Matcher unused) {
        String cmd = _comment.replaceFirst("").trim().toLowerCase();
        if (Move.isGrammaticalMove(cmd)) {
            Move potential = Move.mv(cmd);
            if (potential != null) {
                if (_board.turn().equals(_board.get(potential.from()))) {
                    if (board().isUnblockedMove(
                            potential.from(),
                            potential.to(),
                            potential.spear())) {
                        board().makeMove(Move.mv(cmd));
                    }
                }
            }
        } else {
            throw error("not viable move");
        }
        if (board().winner() != null) {
            if (board().winner().equals(WHITE)) {
                _winner = WHITE;
                reportNote("White wins.", _winner);
            } else {
                _winner = BLACK;
                reportNote("Black wins.", _winner);
            }
        }
    }
    /** The board. */
