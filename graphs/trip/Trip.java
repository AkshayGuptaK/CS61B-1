...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        int vOne = from;
        int vTwo = segment.get(1);

        Road road = _map.getLabel(vOne, vTwo);
        String roadName = road.toString();
        String dir = road.direction().fullName();
        double dist = road.length();
        String roadName2;
        String dir2;
        String last = "";
        for (int i = 2; i < segment.size(); i++) {
            vOne = vTwo;
            vTwo = segment.get(i);
            Road road2 = _map.getLabel(vOne, vTwo);
            roadName2 = road2.toString();
            dir2 = road2.direction().fullName();
            double dist2 = road2.length();
            if (roadName.equals(roadName2) && dir.equals(dir2)) {
                dist += dist2;
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.%n",
                        seq, roadName, dir, dist);
                seq++;
                roadName = roadName2;
                dir = dir2;
                dist = dist2;
            }
            last = _map.getLabel(vTwo).toString();
        }
        System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                seq, roadName, dir, dist, last);
        seq++;
        return seq;
    }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double estimatedDistance(int v) {
            return _map.getLabel(v).dist(_map.getLabel(v), _finalLocation);
        }
