...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            error("Error: have non-empty command set already");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (int dep: _depends.successors(getVertex())) {
            if (_depends.getLabel(dep).isUnfinished()) {
                error("Error: not done rebuilding dependencies");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int dep: _depends.successors(getVertex())) {
            ...
            Integer dependTime = _depends.getLabel(dep).getTime();
            if (dependTime == null || targetTime < dependTime) {
                return true;
            }
        }
        return false;
